...
def Strukturni_predict_fun(train_index, test_index, atribute, output, ModelSTNo):
    
    x_train, x_test = atribute.iloc[train_index,:], atribute.iloc[test_index,:]
    y_train, y_test = output.iloc[train_index,18:], output.iloc[test_index,18:]
    ...
    model3.add(Dense(18, activation='linear'))
    ...
    model4.add(Dense(18, activation='linear'))
