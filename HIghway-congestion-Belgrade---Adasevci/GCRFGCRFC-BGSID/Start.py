...
for train_index,test_index in skf.split(atribute, output):
    ...
    Y_test = Y_test1.iloc[:,:6]
    Y_train = Y_train1.iloc[:,:6]
    y_train_un = y_train_un1.iloc[:,:6]
    
    Y_test_reg = Y_test1.iloc[:,6:]
    Y_train_reg = Y_train1.iloc[:,6:]
    y_train_un_reg = y_train_un1.iloc[:,6:]
